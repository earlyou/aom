...
public class OshiStaticTests {
	...
	void contextLoads() {
		...
		for (OSFileStore fs : fileSystem.getFileStores()) {
			System.out.println(fs.getName()); 																// 로컬 고정 디스크 (C:)
		}
		String dvmd =  dv.get(0).getModel();																// CT120BX500SSD1 (표준 디스크 드라이브)
		double dvsz = Math.round(dv.get(0).getSize()/(1024.0*1024.0*1024.0)*100)/100.0;						// 111.79 GB
